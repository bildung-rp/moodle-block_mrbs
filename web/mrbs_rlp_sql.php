 ...
 */
function mrbs_rlpCheckFree($room_id, $starttime, $endtime, $ignore, $repignore)
{
    global $DB;
    ...
    foreach ($entries as $entry) {
        ...
        $dayurl = new moodle_url('/blocks/mrbs_rlp/web/day.php', $param_ymd);
        $weekurl = new moodle_url('/blocks/mrbs_rlp/web/week.php', array_merge($param_ymd, ['room' => $room_id]));
        $monthurl = new moodle_url('/blocks/mrbs_rlp/web/month.php', array_merge($param_ym, ['room' => $room_id]));

        $err .= "<LI><A HREF=\"" . $viewurl . "\">$entry->name</A>"
                . " ( " . $startstr . ") "
                . "(<A HREF=\"" . $dayurl . "\">" . get_string('viewday', 'block_mrbs_rlp') . "</a>"
                . " | <A HREF=\"" . $weekurl . "\">" . get_string('viewweek', 'block_mrbs_rlp') . "</a>"
                . " | <A HREF=\"" . $monthurl . "\">" . get_string('viewmonth', 'block_mrbs_rlp') . "</a>)";
    }
 ...
 */
function mrbs_rlpDelEntry($user, $id, $series, $all, $roomadminoverride = false)
{
    global $DB;
    ...
    if ($repeat_id > 0 && $DB->count_records('block_mrbs_rlp_entry', ['repeat_id' => $repeat_id]) == 0) {
 ...
 */
function mrbs_rlpCreateSingleEntry($starttime, $endtime, $entry_type, $repeat_id, $room_id, $owner, $name, $type, $description, $oldid = 0, $roomchange = false)
{
    global $DB;
    ...
    if ($endtime > $starttime) {
        if ($oldid) {
            $add->id = $oldid;
            $DB->update_record('block_mrbs_rlp_entry', $add);
            return $oldid;
        } else {
            return $DB->insert_record('block_mrbs_rlp_entry', $add);
        }
    }
 ...
 *   non-zero - The entry's ID
 */
function mrbs_rlpCreateRepeatEntry($starttime, $endtime, $rep_type, $rep_enddate, $rep_opt, $room_id, $owner, $name, $type, $description, $rep_num_weeks, $oldrepeatid = 0)
{
    global $DB;
    ...
    if ($oldrepeatid) {
        $add->id = $oldrepeatid;
        $DB->update_record('block_mrbs_rlp_repeat', $add);
    } else {
        $add->id = $DB->insert_record('block_mrbs_rlp_repeat', $add);
    }
 ...
 */
function mrbs_rlpGetRepeatEntryList($time, $enddate, $rep_type, $rep_opt, $max_ittr, $rep_num_weeks)
{
    $sec = date("s", $time);
    $min = date("i", $time);
    $hour = date("G", $time);
 ...
 *   non-zero - The entry's ID
 */
function mrbs_rlpCreateRepeatingEntrys($starttime, $endtime, $rep_type, $rep_enddate, $rep_opt, $room_id, $owner, $name, $type, $description, $rep_num_weeks, $roomchange = false, $oldid = 0)
{
    ...
    $ret->created = 0;
    $ret->lasttime = null;
    $reps = mrbs_rlpGetRepeatEntryList($starttime, $rep_enddate, $rep_type, $rep_opt, $max_rep_entrys, $rep_num_weeks);
    ...
    $repeatid = 0;
    if ($oldid) {
        $repeatid = $DB->get_field('block_mrbs_rlp_entry', 'repeat_id', ['id' => $oldid]);
    }

    if (empty($reps)) {
        if ($repeatid) {
            // This was a repeat entry, but the entry no longer has any repeats, so delete the repeats (but not this entry)
            $DB->delete_records_select('block_mrbs_rlp_entry', 'repeat_id = :repeatid AND id <> :oldid', compact('repeatid', 'oldid'));
            $DB->delete_records('block_mrbs_rlp_repeat', ['id' => $repeatid]);
        }

        $ret->id = mrbs_rlpCreateSingleEntry($starttime, $endtime, 0, 0, $room_id, $owner, $name, $type, $description, $oldid, $roomchange);
        $ret->repeating = 0;
        $ret->requested = 1;
        $ret->created = 1;
    ...
    $ret->id = mrbs_rlpCreateRepeatEntry($starttime, $endtime, $rep_type, $rep_enddate, $rep_opt, $room_id, $owner, $name, $type, $description, $rep_num_weeks, $repeatid);
    ...
    if ($ret->id) {
        ...
        if ($repeatid) {
            ...
            $oldids = $DB->get_fieldset_sql('SELECT id FROM {block_mrbs_rlp_entry} WHERE repeat_id = ? ORDER BY start_time', [$repeatid]);
        }
        ...
        for ($i = 0; $i < count($reps); $i++) {
            ...
            mrbs_rlpCreateSingleEntry($reps[$i], $reps[$i] + $diff, 1, $ret->id, $room_id, $owner, $name, $type, $description, $updateid, $roomchange);
            $ret->lasttime = $reps[$i];
        ...
        for ($i = count($reps); $i < count($oldids); $i++) {
            $DB->delete_records('block_mrbs_rlp_entry', ['id' => $oldids[$i]]);
        }
    }
    return $ret;
}

/* mrbs_rlpGetEntryInfo()
 *
...
function mrbs_rlpGetEntryInfo($id)
{
    global $DB;
    return $DB->get_record('block_mrbs_rlp_entry', ['id' => $id]);
}

function mrbs_rlpGetRoomArea($id)
{
    global $DB;

    return $DB->get_field('block_mrbs_rlp_room', 'area_id', ['id' => $id]);
}
